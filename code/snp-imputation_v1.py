...
# * Command example : python snp-imputation_v1.py -N sampleID.pileup -I /input/data/ -O /output/data/
###########################################################################################################################
import re
import os
import sys, getopt
    ...
    for line in input:
        ...
        try:
            if arr[0] == 'chrX':
                ...
                # key = 'chrN_position'
                key = arr[0]+'_'+arr[1]
                if snp[key][0] == key:  # Find the same key in PLINK(pad/map) files and pileup file.
...
def cal_ff(sampleID):   # input : sampleID_match_list, sampleID_A, sampleID_B
    ...
    NMA_fraction = (FF[1]/(FF[0]+FF[1]))*100  # non maternal allels fraction = (B/(A+B))*100
...
###########################################################################################################################
def delete_file(sampleID):  # Delete temporary files.
    try:
...
cal_ff(sampleID)
delete_file(sampleID)   # If you do not want to delete temporary files, comment out this line.
print('')
